...
CREATE OR REPLACE FUNCTION public.has_role(user_id UUID, role_name public.user_role)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
AS $$
  SELECT EXISTS(SELECT 1 FROM public.profiles WHERE id = user_id AND user_role = role_name);
$$;