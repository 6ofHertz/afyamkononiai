...
const AdminDashboard = () => {
  ...
  const [isCreatingResource, setIsCreatingResource] = useState(false); // State for resource creation modal
  const [newResourceFormData, setNewResourceFormData] = useState({ // State for new resource form
    title: '',
    description: '',
    category: '',
    url: '',
  });
  const [isSavingResource, setIsSavingResource] = useState(false); // State for resource save button loading
  ...
  const handleCreateResource = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .single(); // Use single() if inserting one row and expecting the inserted row back
      ...
      if (fetchError) {
        console.error('Error fetching updated resources:', fetchError);
      } else {
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="mt-12">
            ...
            <Dialog open={isCreatingResource} onOpenChange={setIsCreatingResource}>
              <DialogTrigger asChild>
                <Button className="mb-6">Create New Resource</Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
            ...
            {resourceError && <p className="text-red-500">Error: {resourceError}</p>}
                  ...
                  <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                    <thead>
                      <tr>
                        ...
                        <th className="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-gray-600 uppercase tracking-wider">URL</th>
                        {/* Add more table headers for other resource fields */}
                        <th className="px-6 py-3 border-b-2 border-gray-300"></th>{/* Actions column */}
                      </tr>
                    </thead>
                    <tbody>
                      {resources.map((resource) => (
                        <tr key={resource.id}>
                          <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{resource.id}</td>
                          <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{resource.title}</td>
                           <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{resource.category}</td>
                            <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200 truncate"><a href={resource.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{resource.url}</a></td>
                          {/* Add more table cells for other resource fields */}
                          <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-right text-sm leading-5 font-medium">
                            {/* Resource action buttons will go here */}
                          </td>
                        </tr>
                      ))}
                    </tbody>
